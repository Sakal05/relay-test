...
import { Hex } from "@openzeppelin/defender-sdk-relay-client/lib/models";
...
async function balance(addr: string) {
  ...
  const balance = await client.relaySigner.call({
...
async function jsonrpc(method: string, payload: string) {
  if (!method) throw new Error(`Missing method`);
  if (!payload) throw new Error(`Missing payload`);
  const result = await client.relaySigner.call({
    method,
...
async function main() {
  try {
    ...
    if (!action) {
      console.error(
        `Usage: node index.js query|get|status|list|send|sign|balance`
      );
      process.exit(1);
    }
    switch (action) {
      ...
      default:
        console.error(
          `Unknown action ${process.argv[2]}, valid actions: query|get|list|send|sign|balance`
        );
        process.exit(1);
    }
  } catch (e) {
